...
from allen.allen_feedforward import FeedForward
...
class CoreferenceResolver(nn.Module):
    def __init__(self,
        ...
        self._context_layer = nn.LSTM(input_size=500, hidden_size=200, num_layers=1, dropout=0.2, bidirectional=True)
        ...
        self._endpoint_span_extractor = EndpointSpanExtractor(self._context_layer.get_output_dim(),
                                                              combination="x,y",
        ...
        self._distance_embedding = nn.Embedding(self._num_distance_buckets, feature_size)
    ...
    def forward(self,  # type: ignore
        ...
        text_mask = aln_util.get_text_field_mask(text).float()
        ...
        flat_top_span_indices = aln_util.flatten_and_batch_shift_indices(top_span_indices, num_spans)

        # Compute final predictions for which spans to consider as mentions.
        # Shape: (batch_size, num_spans_to_keep, 2)
        top_spans = aln_util.batched_index_select(spans,
                                                  top_span_indices,
        ...
        valid_antecedent_indices, valid_antecedent_offsets, valid_antecedent_log_mask = \
            self._generate_valid_antecedents(num_spans_to_keep, max_antecedents, aln_util.get_device_of(text_mask))
        # Select tensors relating to the antecedent spans.
        # Shape: (batch_size, num_spans_to_keep, max_antecedents, embedding_size)
        candidate_antecedent_embeddings = aln_util.flattened_index_select(top_span_embeddings,
                                                                          valid_antecedent_indices)

        # Shape: (batch_size, num_spans_to_keep, max_antecedents)
        candidate_antecedent_mention_scores = aln_util.flattened_index_select(top_span_mention_scores,
        ...
        if span_labels is not None:
            ...
            pruned_gold_labels = aln_util.batched_index_select(span_labels.unsqueeze(-1),
                                                               top_span_indices,
                                                               flat_top_span_indices)

            antecedent_labels = aln_util.flattened_index_select(pruned_gold_labels,
                                                                valid_antecedent_indices).squeeze(-1)
            ...
            coreference_log_probs = aln_util.last_dim_log_softmax(coreference_scores, top_span_mask)
            correct_antecedent_log_probs = coreference_log_probs + gold_antecedent_labels.log()
            negative_marginal_log_likelihood = -aln_util.logsumexp(correct_antecedent_log_probs).sum()
        ...
        for top_spans, predicted_antecedents in zip(batch_top_spans, batch_predicted_antecedents):
            ...
            for i, (span, predicted_antecedent) in enumerate(zip(top_spans, predicted_antecedents)):
                ...
                if antecedent_span in spans_to_cluster_ids:
                    predicted_cluster_id = spans_to_cluster_ids[antecedent_span] #: int = spans_to_cluster_ids[antecedent_span]
                else:
    ...
    def _generate_valid_antecedents(num_spans_to_keep: int,
        ...
        target_indices = aln_util.get_range_vector(num_spans_to_keep, device).unsqueeze(1)

        # Shape: (1, max_antecedents)
        valid_antecedent_offsets = (aln_util.get_range_vector(max_antecedents, device) + 1).unsqueeze(0)
