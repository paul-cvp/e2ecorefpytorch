...
class EndpointSpanExtractor(nn.Module):
    ...
    def forward(self,
        ...
        if self._span_width_embedding is not None:
            ...
            return torch.cat([combined_tensors, span_width_embeddings], -1).squeeze(0)

        if span_indices_mask is not None:
            return (combined_tensors * span_indices_mask.unsqueeze(-1).float()).squeeze(0)
        return combined_tensors.squeeeze(0)
