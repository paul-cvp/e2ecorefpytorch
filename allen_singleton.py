...
#     lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.5, mode="max", patience=2)
#     word_embeddings = [util.load_embedding_dict(emb["path"], emb["size"], emb["format"]) for emb in config["embeddings"]]
#     train_data = cmdata.TrainCorefDataset(config, word_embeddings, "train")
#     val_data = cmdata.TrainCorefDataset(config, word_embeddings, "test")
#
#     model.train()
...
#             model.zero_grad()
#             word_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids = train_data.__getitem__(i)
#             # data [0 -> 4] inputs 5 is training flag [6 -> 8] predictions to be trained against gold = (gold_starts, gold_ends, cluster_ids)
#
#             lr_scheduler.optimizer.zero_grad()
#             word_emb = torch.FloatTensor(word_emb)
#             char_index = torch.LongTensor(char_index)
#             text_len = torch.LongTensor(text_len)
#             speaker_ids = torch.LongTensor(speaker_ids)
#             output_dict = model(word_emb, char_index, text_len, speaker_ids, genre, 1, gold_starts, gold_ends, cluster_ids)
#
#             loss = output_dict["loss"]
#             loss.backward()
#             torch.nn.utils.clip_grad_norm_(parameters, config["max_gradient_norm"])
#             global_step = i+1
...
class Trainer:
    def __init__(self,
                 ...
                 validation_dataset,
                 patience: int = None,
                 validation_metric: str = "+coref_f1",
                 num_epochs: int = 20,
                 ...
                 learning_rate_scheduler: torch.optim.lr_scheduler.ReduceLROnPlateau = None,
        ...
        if patience is None:  # no early stopping
            ...
            raise ValueError('{} is an invalid value for "patience": it must be a positive integer '
        ...
        if increase_or_decrease not in ["+", "-"]:
            raise ValueError("Validation metrics must specify whether they should increase "
        ...
        if not isinstance(cuda_device, int) and not isinstance(cuda_device, list):
            raise ValueError("Expected an int or list for cuda_device, got {}".format(cuda_device))
    ...
    def _batch_loss(self, batch, for_training: bool) -> torch.Tensor:
        ...
        if self._multiple_gpu:
            ...
            if self._cuda_devices[0]>=0:
                word_emb = torch.cuda.FloatTensor(batch[0])
                char_index = torch.cuda.LongTensor(batch[1])
                text_len = torch.cuda.LongTensor(batch[2])
                speaker_ids = torch.cuda.LongTensor(batch[3])
            else:
                word_emb = torch.FloatTensor(batch[0])
                char_index = torch.LongTensor(batch[1])
                text_len = torch.LongTensor(batch[2])
                speaker_ids = torch.LongTensor(batch[3])

            output_dict = self._model(word_emb, char_index, text_len, speaker_ids, batch[4], batch[5], batch[6], batch[7], batch[8])
        try:
            loss = output_dict["loss"]
            if for_training:
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        num_training_batches = len(self._train_data)  # self._iterator.get_num_batches(self._train_data)
        train_generator_tqdm = tqdm.tqdm(self._train_data,
        ...
        for batch in train_generator_tqdm:
            ...
            if self._learning_rate_scheduler:
                self._learning_rate_scheduler.step(batch_num_total)
    ...
    def train(self) -> Dict[str, Any]:
        ...
        try:
            ...
            traceback.print_exc()
            raise ValueError("Could not recover training from the checkpoint.  Did you mean to output to "
        ...
        if validation_metric_per_epoch:
            ...
            metrics["best_validation_{}".format(self._validation_metric)] = best_validation_metric
